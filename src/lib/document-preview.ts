...
import { defaultDocumentLanguageForCountry, parseDocumentLanguage } from "@/lib/language";
import { buildPdfPreviewDocumentData } from "@/lib/pdf-document";
...
export async function documentPreviewResponse(document: Invoice, request: Request): Promise<Response> {
  ...
  const lang = requestedLang ?? defaultDocumentLanguageForCountry(client?.country);
