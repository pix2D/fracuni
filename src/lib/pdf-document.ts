...
// the layout decisions (tax breakdown, legal text, EU formatting, date
...
import { decideServiceVat, serviceLegalTextForLang } from "@/lib/tax-engine";
...
export interface PdfTotals {
  subtotal: string;
  /** null when service VAT is not charged. */
...
function formatDate(iso: string | null, lang: DocumentLanguage): string | null {
  if (!iso) return null;
...
function pick(lang: DocumentLanguage, hr: string | null, en: string | null): string | null {
...
function buildCompanyTaxId(company: Company, lang: DocumentLanguage): { label: string; value: string } {
  if (lang === "en") {
...
function buildTaxIds(client: Client, lang: DocumentLanguage): { label: string; value: string }[] {
...
export function buildPdfDocumentData(input: BuildPdfDataInput): PdfDocumentData {
  ...
  const vatDecision = decideServiceVat({
  ...
  const totals = computeInvoiceTotals(items, currency, { chargeVat: vatDecision.chargesVat, vatRate });
  ...
  const lineItems: PdfLineRow[] = invoice.lineItems.map((li) => {
    ...
    return {
      ...
      vatPercent: vatDecision.chargesVat ? String(vatRate) : null,
  ...
  const legalText = serviceLegalTextForLang(company, vatDecision, lang);
  ...
  return {
    ...
    documentNumber: invoice.documentNumber
      ? isOffer
        ...
        : invoice.documentNumber
      : "",
    showVatColumn: vatDecision.chargesVat,
    company: {
      name: company.name,
...
export function buildPdfPreviewDocumentData(input: BuildPdfPreviewDataInput): PdfDocumentData {
  ...
  const vatDecision = client
    ...
    : null;
  const chargeVat = vatDecision?.chargesVat ?? false;
    ...
    invoice.lineItems.length > 0
      ? invoice.lineItems.map((li) => {
          ...
          return {
            ...
            vatPercent: chargeVat ? String(vatRate) : null,
            unitPrice:
  ...
  const legalText = vatDecision ? serviceLegalTextForLang(company, vatDecision, lang) : null;
